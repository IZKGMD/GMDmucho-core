...
Automated protocol tests and a green CI run are paired with the corresponding real-client fixture. A final real-client claim requires the fixture to exist, be generated by the trace tooling, contain only the expected client family, and pass its release gate.
