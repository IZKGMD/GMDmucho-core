...
$all = new CompatibilityProfile([1, 11, 15, 19, 20, 21, 22]);
assertProfile(true, $all->isAll(), 'all profile detected');

foreach ([1, 11, 15, 19, 20, 21, 22] as $version) {
    assertProfile(
        true,
...
assertProfile('1.5', ClientVersion::fromValues(15, 0)->label(), 'GD 1.5 label detected');
assertProfile(false, ClientVersion::fromValues(15, 0)->usesGjp2(), 'GD 1.5 does not use GJP2');
...
assertProfile(true, (new CompatibilityProfile([15]))->allows(ClientVersion::fromValues(15, 0)), '1.5 profile allows GD 1.5');
...
$_ENV['MUCHO_GD_VERSIONS'] = '11,15,22';
...
assertProfile(true, $custom->allows(ClientVersion::fromValues(15, 0)), 'custom profile allows 1.5');
